...
## Material Field Classification Reference

Quick reference table for all Material entity fields and their input classification:

| Campo | Tipo | Categoria | Auto-preenchível? | Descrição |
|-------|------|-----------|------------------|-----------|
| `id` | UUID | System | ❌ | Auto-generated by database |
| `titulo` | VARCHAR(255) | **AI-Assisted** | ✅ Yes (0.85–0.95 confidence) | Material title; extracted via OCR; always editable |
| `descricao` | TEXT | **Manual-Only** | ❌ Never (prevent hallucinations) | Detailed description; must be manually entered by donor |
| `disciplina` | ENUM | **AI-Assisted** | ✅ Yes (high confidence) | Subject/discipline; extracted from content analysis |
| `nivel_ensino` | ENUM | **AI-Assisted** | ✅ Yes (high confidence) | Education level; extracted from material characteristics |
| `ano` | INTEGER | **AI-Assisted** | ✅ Yes (medium confidence) | Grade/year; extracted if visible; NULL for SUPERIOR |
| `sistema_ensino` | ENUM | **AI-Assisted** | ✅ Yes (medium confidence) | Curriculum system; inferred from publisher/content |
| `estado_conservacao` | ENUM | **AI-Assisted** | ✅ Yes (high confidence) | Conservation state; assessed from image quality |
| `data_publicacao` | INTEGER/DATE | **AI-Assisted** | ✅ Yes (medium confidence) | Publication year; extracted from cover/metadata if visible |
| `imagem_url` | VARCHAR(500) | System | ❌ | Set by backend after image promotion to permanent storage |
| `upload_id` | VARCHAR(100) | System | ❌ | Temporary upload tracking ID; used for deduplication |
| `doador_id` | UUID | System | ❌ | Foreign key; set from authenticated user context |
| `status` | ENUM | System | ❌ | Lifecycle state; initialized as DISPONIVEL; managed by backend |
| `cidade` | VARCHAR(100) | Manual/Profile | ⚠️ Optional | Donor location; normalized; can use profile default or manual input |
| `bairro` | VARCHAR(100) | Manual/Profile | ⚠️ Optional | Donor neighborhood; normalized; can use profile default or manual input |
| `created_at` | TIMESTAMP | System | ❌ | DB insert timestamp |
| `updated_at` | TIMESTAMP | System | ❌ | DB update timestamp |

**Legend**:
- ✅ **AI-Assisted**: Gemini will attempt to extract/infer this field with confidence scoring
- ❌ **Manual-Only / System**: User must enter manually OR backend generates automatically
- ⚠️ **Hybrid**: Can use defaults from user profile or accept manual override

**Key Rules**:
- `descricao` is **NEVER** auto-populated to prevent AI hallucinations (see Gemini prompt template)
- `titulo` is **always editable** even when high-confidence (users can override)
- `ano` defaults to NULL for SUPERIOR level (no year filtering for upper education)
- `cidade` and `bairro` are **normalized** before insertion (uppercase, NFD→ASCII)
